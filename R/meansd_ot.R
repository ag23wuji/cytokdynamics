...
#' meansd_ot(
#'   eu_stock_df |> mutate(stock = factor(stock)),
#'   x_axis = "year",
#'   y_axis = "price",
#'   color = "stock",
#'   ggplot_objects = list(
...
#'
#' @export
meansd_ot <- function(data, x_axis = NULL, y_axis = NULL, color = NULL,
                      ggplot_objects = list(ggplot2::geom_line()), group_args = list()) {
  ...
  if (is.null(color)) {
    ...
    p <- ggplot2::ggplot(data, ggplot2::aes(x = .data[[x_axis]], y = .data[[paste0(y_axis, "_mean")]])) +
      ggplot2::geom_point() +
      ggplot2::geom_errorbar(ggplot2::aes(
        ymin = .data[[paste0(y_axis, "_mean")]] - .data[[paste0(y_axis, "_sd")]],
        ymax = .data[[paste0(y_axis, "_mean")]] + .data[[paste0(y_axis, "_sd")]]
      ))
  } else {
    data <- data |>
    ...
    p <- ggplot2::ggplot(data, ggplot2::aes(x = .data[[x_axis]], y = .data[[paste0(y_axis, "_mean")]], color = .data[[color]])) +
      ggplot2::geom_point() +
      ggplot2::geom_errorbar(ggplot2::aes(
        ymin = .data[[paste0(y_axis, "_mean")]] - .data[[paste0(y_axis, "_sd")]],
        ymax = .data[[paste0(y_axis, "_mean")]] + .data[[paste0(y_axis, "_sd")]]
